...
c               bL=hstartest+bR
       ...
       !if ((ql(1,i)<roe_depth) .and. (qr(1,i-1)<roe_depth)) then
         ! use augmented solver only if depth is shallow on both sides:

       if ((ql(1,i)<roe_depth) .or. (qr(1,i-1)<roe_depth)) then
         ! use augmented solver only if depth is shallow on either side:
...
!515         format("++sw",4e25.16)
         enddo
          ...
          ! simple f-wave solver if depth > roe_depth:
          ...
          del2 = hR*uR**2 - hL*uL**2 + g*dsqrt(hL*hR) * 
     &           ((hR+bR) - (hL+bL))
